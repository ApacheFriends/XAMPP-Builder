...
developmentDirectories = [ 'build', 'include', 'man', 'manual', 'info', 'share/doc', 'lib/pkgconfig', 'share/man' ]
developmentExtensions = [ '.a', '.la' ]
...
class Component(object):
	...
	@property
	def isBuild(self):
		if os.path.isdir(self.buildPath) and os.path.isfile(self.manifestPath):
			return True
		return False
	...
	def manifestPath(self):
		return os.path.join(self.config.buildsPath, self.name.lower()+'.manifest')

	@property
	def resourcesPath(self):
		return self._resourcesPath
	
	def isPathDevelopmentOnly(self, path):
		
		(path, ext) = os.path.splitext(path)
		if ext in developmentExtensions:
			return True
		
		for devDir in developmentDirectories:
			if path.startswith(os.path.join(self.config.prefixPath, devDir)):
				return True
